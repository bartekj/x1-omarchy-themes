...
`hyprland.lua`, `neovim.lua`, and `vscode.json` dropped. Omarchy regenerates
equivalents from `colors.toml` — border colors survive via the
`hyprland_active_border`/`hyprland_inactive_border` keys — but the custom
geometry (gaps, rounding, blur, group bar) and the base16 Neovim theme /
VS Code extension pin would be lost. Bar switcher widgets
(`x1-theme-status`/`-next`) are optional — they need the `shell.json`
entries described in "Bar cockpit" below.
